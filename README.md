...
Made with love by the Book Publication Workflow Team
...
- **Solution:**
  - Ensure your `PYTHONPATH` includes the project root and `src/` directory.
  - Run scripts from the project root, not from inside subfolders.
  - Double-check that all required files exist and are named correctly.

### 2. API Key/Provider Errors
- **Symptom:** Errors from OpenAI, Anthropic, or Gemini APIs (e.g., authentication failed, invalid key, or missing key).
- **Solution:**
  - Make sure your `.env` file is present and contains valid API keys for all providers you intend to use.
  - Double-check the variable names in `.env` match those expected in `config/settings.py`.
  - Restart your environment after updating `.env`.

### 3. RL Model State Dict Size Mismatch
- **Symptom:**
  - `RuntimeError: Error(s) in loading state_dict for ...`
  - "size mismatch for ... weight"
- **Solution:**
  - Old or incompatible model files may be present in the `models/` directory.
  - Delete all `.pth` files in `models/` and re-run the workflow to regenerate them.

### 4. Playwright/Selenium/Browser Automation Issues
- **Symptom:**
  - Errors about missing browsers, Playwright not installed, or browser launch failures.
- **Solution:**
  - Run `playwright install chromium` after installing dependencies.
  - Ensure you have the latest version of Playwright and Chromium.
  - If running in a headless server or CI, ensure all required system dependencies for Chromium are installed.

### 5. Voice Processing Errors
- **Symptom:**
  - Errors related to `pyaudio`, `pyttsx3`, or `speech_recognition` (e.g., missing device, library not found).
- **Solution:**
  - Make sure all voice dependencies are installed (`pyaudio`, `pyttsx3`, `speechrecognition`).
  - On Linux, you may need to install system packages like `portaudio` (`sudo apt-get install portaudio19-dev`).
  - Check your microphone and audio device permissions.

### 6. ChromaDB/Database Issues
- **Symptom:**
  - Errors about missing or corrupt ChromaDB files, or database connection failures.
- **Solution:**
  - Delete the `chroma_db/` directory and let the system recreate it.
  - Ensure you have write permissions to the data directories.

### 7. General Setup Issues
- **Symptom:**
  - Errors during `pip install`, missing packages, or version conflicts.
- **Solution:**
  - Use the provided `requirements.txt` and create a fresh virtual environment.
  - If you see version conflicts, try upgrading pip (`pip install --upgrade pip`) and reinstalling dependencies. 